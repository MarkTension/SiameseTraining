...
def create_base_network(input_shape):
    ...
    x = Lambda(lambda x: tf.reduce_mean(x, 3))(input)
    x = Flatten()(x)
...
bare_model = create_base_network(input_shape) # create new model
bare_model.set_weights(weights) # place weights

frozen_graph = freeze_session(K.get_session(),
                              output_names=[out.op.name for out in bare_model.outputs])
...
vars = ['dense_1/kernel', 'dense_1/bias', 'dense_2/kernel', 'dense_2/bias', 'dense_3/kernel', 'dense_3/bias'] # , 'lambda_2/Sqrt'
